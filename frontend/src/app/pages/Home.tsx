        ...
        <div className="bg-[#eef4ed] rounded-2xl shadow-2xl p-6 border border-[#8da9c4]/30">
            ...
            <div>
              ...
              <div className="flex items-center gap-2.5">
                <div className="flex items-center gap-2">
                  <div className={`w-2.5 h-2.5 rounded-full ${policy?.claim ? "bg-red-500" : "bg-emerald-500"}`}></div>
                  <p className="text-lg font-semibold text-[#0b2545]">
                    {policy?.claim ? "Disruption" : "Protected"}
                  </p>
                </div>
                <div className={`px-3 py-1 rounded-lg ${policy?.claim ? "bg-red-500" : "bg-emerald-500"}`}>
                  <p className="text-xs font-bold text-white">Live</p>
                </div>
              </div>
            </div>
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg ${policy?.claim ? "bg-gradient-to-br from-red-500 to-red-700" : "bg-gradient-to-br from-[#134074] to-[#13315c]"}`}>
          ...
          <div className="pt-4 border-t border-[#8da9c4]/40 grid grid-cols-2 gap-4">
            ...
            <div className="text-right">
              ...
              <p className="text-sm font-semibold text-[#0b2545]">{getNextSunday()}</p>
            </div>
          </div>
        </div>
      </div>

      {/* Claim Alert Card - only shows when claim exists */}
      {policy?.claim && (
        <div className="px-6 mb-6">
          <div className="bg-red-100 border-2 border-red-400 rounded-2xl p-5 animate-pulse">
            <div className="flex items-center gap-3">
              <span className="text-2xl">🚨</span>
              <div>
                <p className="font-bold text-red-700 text-base">Claim Auto-Approved</p>
                <p className="text-red-600 text-sm">₹{policy?.claim?.amount} credited • Claim #{policy?.claim?._id?.slice(-3) || "998"}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-[#eef4ed] rounded-2xl p-4 border border-[#8da9c4]/40 shadow-sm">
            ...
            <div className="mt-2.5 flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${(weather.rainfall || 0) > 40 ? "bg-red-500" : "bg-amber-500"}`}></div>
              <p className={`text-xs font-semibold ${(weather.rainfall || 0) > 40 ? "text-red-600" : "text-amber-600"}`}>
                {(weather.rainfall || 0) > 40 ? "High" : "Moderate"}
              </p>
            </div>
          </div>
          <div className="bg-[#eef4ed] rounded-2xl p-4 border border-[#8da9c4]/40 shadow-sm">
            ...
            <div className="mt-2.5 flex items-center gap-1.5">
              <div className={`w-1.5 h-1.5 rounded-full ${(weather.aqi || 0) > 300 ? "bg-red-500" : "bg-orange-500"}`}></div>
              <p className={`text-xs font-semibold ${(weather.aqi || 0) > 300 ? "text-red-600" : "text-orange-600"}`}>
                {(weather.aqi || 0) > 300 ? "Severe" : "Moderate"}
              </p>
            </div>
          </div>
          <div className="bg-[#eef4ed] rounded-2xl p-4 border border-[#8da9c4]/40 shadow-sm">
      ...
      <div className="px-6 mb-6">
        <div className="relative bg-gradient-to-r from-[#134074] to-[#13315c] rounded-2xl p-5 text-white shadow-lg overflow-hidden border border-[#8da9c4]/30">
          ...
          <div className="relative z-10 flex items-start gap-3.5">
            ...
            <div>
              <p className="font-bold text-base mb-1.5">Heavy Rain Alert</p>
              <p className="text-sm text-white/95 leading-relaxed">Expected Thursday. Work extra today — your Thursday income is fully protected under your ₹{policy.weeklyPremium || 36} premium.</p>
            </div>
          </div>
        </div>
      </div>
